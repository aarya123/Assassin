...
if(!array_key_exists('password', $_POST) || !array_key_exists('email_address', $_POST) ||
   ...
   !array_key_exists('height', $_POST) || !array_key_exists('locations', $_POST) || !array_key_exists('name', $_POST)) {
	print json_encode(array('error' => 'usage: post password, email_address, name, sex, age, race, height, locations, name (comma separated)'));
	exit();
}
...
try {
	...
	$stmt = $dbh->prepare('INSERT INTO users (password_hash, email_address, sex, age, race, height, location, name) VALUES (:password_hash, :email_address, :sex, :age, :race, :height, :location, :name)');
	...
	$stmt->bindParam(':location', $_POST['locations']);
